...
本仓库自动维护上述候选列表，明确用于 ACL 白名单。`dev` 分支当前试验全国分层准入：除要求当前 BGP Origin 属于中国电信、中国移动或中国联通外，还必须存在归属于同一家运营商的 APNIC 覆盖登记。

APNIC 最具体登记不再作为逐地址准入边界：其中的客户、项目和历史子登记会保留审计，只有明确指向其他运营商，或命中云厂商 CIDR、明确非普通用户侧用途、独立资源持有人、独立 route origin、强 MOAS 等既有强证据时才会删除。规则不枚举省、市或县级分支名称，所有地区使用完全相同的全国规则。它优先降低误收，同时以硬性 CIDR 膨胀上限保证 ACL 可部署性。
...
flowchart TD
    ...
    Y --> V
    C --> V
    V --> AA["全国分层准入：三网 Origin + 同运营商 APNIC 覆盖登记"]
    Q --> AA
    C --> AA
    R --> AB["最具体登记：强证据排除与运营商冲突拒绝"]
    AB --> AA
    AA --> J["最终三网地址池"]
...
- 全国分层准入在既有排除之后执行：每个运营商候选地址必须落在由 APNIC 全国通用名称规则识别为同一家运营商的某个覆盖 `inetnum` 内。更具体的客户、项目或历史登记本身不再挖洞；明确的运营商冲突仍拒绝，云、IDC、独立资源主体等强证据仍由既有规则按最具体范围删除。配置不得通过枚举地方分支名称提高单一地区召回率。
...
分层准入直接作用于全国地址池，省级文件只是全国最终结果与 `ip2region` 地域范围的交集，不存在浙江专用准入逻辑。manifest 的 `pre_operator_parent_registration_admission`、`operator_parent_registration_denials` 和 `operator_parent_registration_admissions` 阶段记录全国试验规模；`operator_registration_admission` 另外记录准入模式、前后 CIDR 数、膨胀率及 2.0 倍硬上限。
