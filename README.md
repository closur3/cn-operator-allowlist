...
这里的“终端用户接入网”主要指住宅宽带和移动数据网络，不包括 IDC、云计算、CDN、企业专线、机构统一出口和专用骨干。结果是依据当前公开 BGP 与注册数据生成的 **best-effort 候选集**，不是地址实际用途、归属或安全性的保证。使用方应按自己的风险边界叠加认证、限速和审计策略，而不应把 IP 白名单当作唯一信任依据。
...
| 路径 | 内容 |
| --- | --- |
| `lists/ipv4/cn.txt` | 三网 IPv4 全国合表 |
| `lists/ipv4/chinatelecom.txt` | 中国电信 IPv4 |
| `lists/ipv4/chinamobile.txt` | 中国移动 IPv4 |
| `lists/ipv4/chinaunicom.txt` | 中国联通 IPv4 |
| `lists/ipv4/provinces/<pinyin>.txt` | IPv4 省级合表 |
| `lists/ipv6/cn.txt` | 三网 IPv6 全国合表 |
| `lists/ipv6/chinatelecom.txt` | 中国电信 IPv6 |
| `lists/ipv6/chinamobile.txt` | 中国移动 IPv6 |
| `lists/ipv6/chinaunicom.txt` | 中国联通 IPv6 |
| `lists/ipv6/provinces/<pinyin>.txt` | IPv6 省级合表 |
| `lists/manifest.json` | 公开文件的内容标识、CIDR 数和 SHA-256 |

省级文件覆盖中国大陆 31 个省级行政区，使用拼音文件名，例如 `beijing.txt`、`guangdong.txt`、`shaanxi.txt` 和 `xinjiang.txt`。每个文本文件每行一个规范化 CIDR，按地址排序，文件内部无重叠。

`lists/manifest.json` 的 `content_id` 标识整套公开列表的确定内容；下载后可用其中每个文件的 SHA-256 校验完整性。

## 收录范围

| 网络类型 | 策略 |
| --- | --- |
| 三网住宅宽带和移动数据地址 | 满足相应地址族的准入条件后保留 |
...
- 中国电信准入范围在每次构建时从完整 APNIC `inet6num` 数据库发现，匹配固定宽带和移动接入的精确登记描述；更具体的 APNIC 登记会覆盖较宽的登记。
- 中国移动和中国联通使用代码中审计过的全国接入地址边界。
- `generator/config/ipv6-province-prefixes.json` 是 31 个省级 IPv6 分配的仓库内事实表，同时区分运营商、固网与移动网；公开列表不再拆分接入类型。
- 每个运营商列表必须完全落入省级分配表。三网互不重叠，运营商并集和省级并集都必须严格等于 `lists/ipv6/cn.txt`。

IPv6 的详细准入来源、Origin、拒绝原因和输入摘要作为 CI 审计产物保存，不写入精简的公开 manifest。
...
GitHub Actions 还使用 `actions/checkout@v6`、`actions/setup-go@v6` 和 `actions/upload-artifact@v4` 作为自动化依赖；它们不参与地址归属判定。生成器只依赖 Go 标准库，没有第三方 Go 模块。

## 自动更新与验证

[更新工作流](.github/workflows/update.yml) 每天 UTC 08:08 运行，也支持手动触发。仓库以 `main` 为正式主线、`dev` 为开发线：代码、规则、文档和工作流变更先在 `dev` 验证，`main` 发布正式列表。
